...
public class LinkedList<T>{
	 ...
	 */
	public T popLast() {
		T lastNodeValue = lastNode.getValue();
	...
	public int find(int from, T value) {
		...
		if(from==size-1) {
			return lastNode.compareTo(value)==0 ? size-1 : -1;
		}
	...
	public void reverse() throws CloneNotSupportedException {
		...
		Node<T> holderNode = firstNode.getNextNode();
		Node<T> holdedNode = firstNode;
		firstNode.setNextNode(null);
		
		for(int i=0;i<size-2;i++) {
			Node<T> tempNode = holderNode.getNextNode();
			holderNode.setNextNode(holdedNode);
			holdedNode=holderNode;
			holderNode=tempNode;
		}
		holderNode.setNextNode(holdedNode);
